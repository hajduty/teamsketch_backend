...
/**
 * Map to track active WebSocket connections by room and userid
 * @type {Map<string, Map<string, Set<uws.WebSocket<User>>>>}
 */
const connectionsByRoomAndUser = new Map()
...
export const registerYWebsocketServer = async (app, pattern, store, checkAuth, { redisPrefix = 'y', initDocCallback = () => {} } = {}) => {
  ...
  app.ws(pattern, /** @type {uws.WebSocketBehavior<User>} */ ({
    ...
    open: async (ws) => {
      ...
      // Track connection
      const roomMap = connectionsByRoomAndUser.get(user.room) || new Map()
      const userSet = roomMap.get(user.userid) || new Set()
      userSet.add(ws)
      roomMap.set(user.userid, userSet)
      connectionsByRoomAndUser.set(user.room, roomMap)
    ...
    close: (ws, code, message) => {
      ...
      const roomMap = connectionsByRoomAndUser.get(user.room)
      if (roomMap) {
        const userSet = roomMap.get(user.userid)
        if (userSet) {
          userSet.delete(ws)
          if (userSet.size === 0) {
            roomMap.delete(user.userid)
            if (roomMap.size === 0) {
              connectionsByRoomAndUser.delete(user.room)
            }
          }
        }
      }
...
/**
 * Close all WebSocket connections for a specific user in a specific room
 * @param {string} targetUserId
 * @param {string} targetRoomId
 * @return {boolean} true if any connections were closed
 */
export const closeConnectionByUserAndRoom = (targetUserId, targetRoomId) => {
  let closedAny = false
  const roomMap = connectionsByRoomAndUser.get(targetRoomId)
  if (roomMap) {
    const userSet = roomMap.get(targetUserId)
    if (userSet) {
      ...
      if (roomMap.size === 0) {
        connectionsByRoomAndUser.delete(targetRoomId)
      }
    }
  }
  if (!closedAny) {
    console.log(`No active connection found for user ${targetUserId} in room ${targetRoomId}`)
  }
  return closedAny
}
